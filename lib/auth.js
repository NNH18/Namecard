(function (root, factory) {
  ...
  class AuthManager {
    constructor({ client, sessionStore, db = null }) {
      this.client = client; this.db = db; this.sessionStore = sessionStore || new ((globalThis.BCardLib?.sessionStore?.BrowserMemorySessionStore) || class { constructor() { this.value = null; } async save(value) { this.value = value; } async load() { return this.value; } async clear() { this.value = null; } })();
      this.session = null; this.listeners = new Set(); this.epoch = 0; this.controller = null; this.refreshTimer = null;
    }
    ...
    async refreshSession() {
      ...
      try { return await this.applySession(await this.client.refresh(this.session.refresh_token), epoch); }
        ...
        if (epoch === this.epoch) {
          this.session = null; this.client.setAccessToken(""); await this.sessionStore.clear(); await this.db?.deleteSession?.(SESSION_KEY); this.emit("SIGNED_OUT", { reason: "SESSION_EXPIRED", error });
        }
        return null;
      }
    }
    ...
    async restore() {
      ...
      catch (error) { await this.sessionStore.clear(); await this.db?.deleteSession?.(SESSION_KEY); if (epoch === this.epoch) { this.session = null; this.client.setAccessToken(""); this.emit("SIGNED_OUT", { reason: "SESSION_EXPIRED" }); } return null; }
    }
    async signOut() {
