(function (root, factory) {
  ...
  class SupabaseClient {
    constructor({ url, anonKey, fetchImpl } = {}) {
      ...
      if (!url || !anonKey || !fetcher) throw new Error("Supabase client config chưa đầy đủ");
      this.url = url.replace(/\/$/, ""); this.anonKey = anonKey; this.fetch = fetchImpl ? fetcher : fetcher.bind(globalThis); this.accessToken = "";
    }
    ...
    rest(table, { method = "GET", query = "", body, headers = {}, signal } = {}) {
      ...
      if (String(method).toUpperCase() !== "GET" || body !== undefined) throw Object.assign(new Error("Private table mutation phải đi qua approved RPC"), { code: "DIRECT_DML_FORBIDDEN" });
